...
describe('Request/Response', (): void => {
    ...
    describe('blpSession start successfully', (): void => {
        ...
        describe('request data arrives in 3 chunks', (): void => {
            beforeEach((): void => {
                ipc.once('wait-to-request', (data: any): void => {
        ...
        describe('request error out immediately', (): void => {
            // Set instructions
            before((): Promise<void> => {
                return ipc.emitAcknowledge('set-instructions', { request: ['generateError'] });
            });
        ...
        describe('session terminated before request arrives', (): void => {
            beforeEach((): void => {
        ...
        describe('session terminated when request arrives', (): void => {
            beforeEach((): void => {
                ipc.once('wait-to-request', (data: any): void => {
                    ipc.emit('terminate-session');
                });
        ...
        describe('request data arrives in 1 chunk then session is terminated', (): void => {
            beforeEach((): void => {
                ipc.once('wait-to-request', (data: any): void => {
                    ipc.emit(util.format('request-%d-partial', data.cid));
                    ipc.emit('terminate-session');
